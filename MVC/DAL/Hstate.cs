...
namespace LEANJU.DAL
{
	...
	public partial class Hstate
	{
		...
		public bool Exists(string Hno)
		{
			...
			strSql.Append("select count(1) from Hstate");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into Hstate(");
			strSql.Append("Hno,isRent,Intime,Outtime)");
			strSql.Append(" values (");
			strSql.Append("@Hno,@isRent,@Intime,@Outtime)");
			SqlParameter[] parameters = {
					new SqlParameter("@Hno", SqlDbType.NVarChar,20),
					new SqlParameter("@isRent", SqlDbType.NVarChar,5),
					new SqlParameter("@Intime", SqlDbType.DateTime),
					new SqlParameter("@Outtime", SqlDbType.DateTime)};
			parameters[0].Value = model.Hno;
			parameters[1].Value = model.isRent;
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update Hstate set ");
			strSql.Append("isRent=@isRent,");
		...
		public bool Delete(string Hno)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from Hstate ");
			strSql.Append(" where Hno=@Hno ");
			SqlParameter[] parameters = {
					new SqlParameter("@Hno", SqlDbType.NVarChar,20)			};
			parameters[0].Value = Hno;
		...
		public LEANJU.Model.Hstate GetModel(string Hno)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 Hno,isRent,Intime,Outtime from Hstate ");
			strSql.Append(" where Hno=@Hno ");
			SqlParameter[] parameters = {
					new SqlParameter("@Hno", SqlDbType.NVarChar,20)			};
			parameters[0].Value = Hno;

			LEANJU.Model.Hstate model=new LEANJU.Model.Hstate();
		...
		/// 得到一个对象实体
		/// </summary>
		public LEANJU.Model.Hstate DataRowToModel(DataRow row)
		{
			LEANJU.Model.Hstate model=new LEANJU.Model.Hstate();
			if (row != null)
			{
				...
				if(row["isRent"]!=null)
				{
					model.isRent=row["isRent"].ToString();
				}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select Hno,isRent,Intime,Outtime ");
			strSql.Append(" FROM Hstate ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from Hstate T ");
